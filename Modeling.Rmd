...
```{r}
regression2 <- 
  glm(Call_Count ~  name + dotw + Temperature, family = "poisson", data = EMS_Call.Train)
```

```{r}
regression3 <- 
  glm(Call_Count ~  name + hour(interval60) + dotw + Temperature, 
     family = "poisson", data = EMS_Call.Train)
```

```{r}
regression4 <- 
  glm(Call_Count ~  name + GEOID +  hour(interval60) + dotw + Temperature +
...
```{r}
regression5 <- 
  glm(Call_Count ~  name + GEOID + hour(interval60) + dotw + Temperature + Percipitation +
                   lagHour + lag2Hours +lag3Hours +lag12Hours + lag1day,
     family = "poisson",
     data = EMS_Call.Train)
```
...
week_predictions <- 
  EMS_Call.Test.weekNest %>% 
    mutate(A_Time_FE = map(.x = data, fit = regression1, .f = model_pred),
           ...
           E_Time_Space_FE_timeLags_weather = map(.x = data, fit = regression5, .f = model_pred))
...
```{r}
filter(week_predictions, Regression == "E_Time_Space_FE_timeLags_weather") %>% 
  unnest() %>% 
  st_sf() %>%
  ...
  gather(Variable, Value, -uniqueID, -geometry) %>%
    ...
    ggplot() + 
      ...
      scale_fill_viridis() +
      labs(title="Mean Absolute Error by Regression and grid") +
