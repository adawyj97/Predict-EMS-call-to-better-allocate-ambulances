...
# 1. Introduction 
In the emergency medical service (EMS) system, the time of responding the call is always crucial for saving people’s lives. In Virginia Beach, there are 10 volunteer rescue squads, but more than 10% of EMS calls are delayed in each year. Though the control center send the closest ambulance to the scene, the unpredicted locations of calls still make the ambulance driver spend a long time on the way. According to the annual report of Virginia Beach EMS, from receiving calls to arriving at the scene, the driver's average response time was more than 9 min, and the responding time kept increasing from 2017 to 2018. Based on the above situation, our project aims to minimize the time from “notifying driver” to “arriving at scene”. 
We would like to  increase the transparency and efficiency of the EMS dispatch system by empowering the ambulance drivers to have more decision-making power. For our use case, the driver would have a general sense about the current call pattern around the city by checking our predicted risk map at 1-hour time interval, and then instead of staying at dispatch center to wait for the call, he could stay around the high risk location in advance. Our multi-driver communication also allows the driver to know the general location of other drivers, 

# 2. Data 
## 2.1 Gathering Data 
We gathered data from multiple perspectives to explore the potential relationships between the number of EMS calls and other features. The predictors included the categories of time lag, demographic features, spatial characteristics, and the other external characteristics of Virginia Beach. The main dataset was the Virginia Beach EMS calls from 2010 to 2018, which was acquired from Virginia Beach Open data. The other supplement datasets were from Tidy Census, Iowa Environment Mesonet, Virginia Road and Esri Open Data.
