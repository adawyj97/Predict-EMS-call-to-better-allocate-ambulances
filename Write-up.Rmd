...
We visualized the EMS call counts by hours of the day through the 9 weeks in time series plot. The black lines separate the plot into weeks. The testing and training data are differentiated by colors. 
Since EMS call is rare and usually caused by accidental events, the data distribution does not have a very clear pattern. According to the timeline, the differences between peak and valley are significantly large, which further indicates the occasion of EMS calls. Though there are many noises associated with the line, one can still observe that each peak represents a day, and the daytime usually has more calls. Except the week 2, the extremely high peak usually happens to a day or a few days of the week. Only in week two, the peaks are almost at the same level. 
...
Next, we aggregated EMS call counts by hours across days of the week and visualized it by hotspot matrix. The matrix indicates that fewer calls are made during the early morning before 6 A.M. The call counts start to increase from 7 A.M. and reach the rush hours at 10 A.M.  to 11 A.M. Then the call counts keep decreasing until midnight. Money, Tuesday and Thursday have around 80 calls during the rush hours, and especially for Monday and Tuesday, the general call counts are larger than that of other days. The rest of four days still follow the general daily pattern, but do not have any extremely high or low counts. 
[call count hotspot by week of the day]
We visualized the correlations between EMS call counts and time lags. The absolute values of correlations are around 0.4 to 0.5 except for the 4-hour lag. In fact, for the first four hours, despite the minor fluctuation between the 2-hour lag and the 3-hour lag,  the value of the correlation dropped from 0.45 to 0.19. The significant reduction also shows a decreasing tendency of the correlation with additional hours. The correlations are stronger again in 12-hour lag and 1-day lag. The 12-hour lag is the only lag with a negative correlation, and we believe it corresponds to the pattern that is displayed in the time series plot: the daytime and nighttime have opposite call count patterns. 
[correlation of time lag]

## 3.2 Call count variation across space
We then explored the spatial pattern of EMS call counts across the Virginia Beach. We visualized the call counts of each grid cell to see whether there are any spatial autocorrelations or specific patterns that may potentially affect our prediction. Since the hour is the time interval of our prediction, we made the small multiple maps below to show the spatial distribution of the sum of three-month EMS call counts per grid per hour. Generally, the calls take place in northern Virginia Beach. In the early morning, the call counts are at a low level and in an even distribution at the northern Virginia Beach. Until time shifts to the daytime,  the general call counts increase,  but there are still no cell grids with extremely high call counts (Except the 12 P.M.). After 6 P.M., the easternmost grid have significantly high call counts than other grid cells do.  
[sum of call counts by grid by hour]
We visualized the aggregated call counts by week through the three months. A large number of call counts form a horizontal line at the northern Virginia Beach, and the call counts gradually outward expand from the line. According to the map, the horizontal line with more call counts is the main road of Virginia Beach, which reflects that the spatial distribution of EMS calls may relate to the road distribution. 
[sum of call counts by grid by week]
...
```{r}
Neighborhoods %>%
  left_join(neighborhood_MAE, by = c('name')) %>% 
