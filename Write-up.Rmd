...
```{r, results="hide"}
EMS_Calls <- st_read('C:\\Users\\Xiaoran Wang\\Desktop\\MUSA507\\FinalProject\\tem_507\\Predict-EMS-call-to-better-allocate-ambulances\\EMS_calls1.geojson') %>%
  st_transform(2284)
...
```{r}
Train_plot <- filter(EMS_Call.panel, week < 28)
Test_plot <- filter(EMS_Call.panel, week >= 28)
```
...
mondayMidnight <- 
  ...
  rbind(
    mutate(Train_plot, label = "Training"), 
    mutate(Test_plot, label = "Testing")) %>%
