...
```{r}
anim_save("EMS Calls Virginia Beach", EMS_Call_animation, duration=24)
```
...
ggplot() + 
  geom_sf(data=fishnet) +
  geom_sf(data=fishnet_centroid) +
  labs(title = "Virginia Beach fishnet centroid") +
  mapTheme()
```
...
census_fishnet <- 
  fishnet_centroid %>% 
  st_join(vbCensus, st_intersects) %>% 
  st_set_geometry(NULL)
```
...
census_fishnet_wide <- 
  fishnet_centroid %>% 
  st_join(vbCensus_wide, st_intersects) %>% 
  st_set_geometry(NULL)
```
