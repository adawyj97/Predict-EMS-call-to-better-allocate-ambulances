...
```{r}
census_search <- load_variables(2017, "acs5", cache = TRUE)
View(census_search)
```

```{r}
=======
>>>>>>> af093fef032851fa87c5f7c4164e744fca96d19d
vbTracts <- 
  vbCensus %>%
  as.data.frame() %>%
...
EMS_Call_final <- EMS_Call_final[is.na(EMS_Call_final$name) != TRUE,] 
```
...
```{r}
ggplot() +
  geom_sf(data=accident_net, aes(fill = countAccident)) +
  mapTheme()
```
