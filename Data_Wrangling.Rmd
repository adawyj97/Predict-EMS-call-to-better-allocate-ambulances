...
EMS_Call.template <- 
  EMS_fishnet %>%
  filter(week >= 22 & week <= 24 | week >= 25 & week <= 29)
```

```{r}
length(unique(EMS_Call.template$interval60)) * length(unique(EMS_Call.template$uniqueID))
```

```{r}
study.panel <- 
  expand.grid(interval60 = unique(EMS_Call.template$interval60), 
              uniqueID = unique(EMS_Call.template$uniqueID)) 
...
```{r}
EMS_Call.Train <- filter(EMS_Call.panel, week < 27)
EMS_Call.Test <- filter(EMS_Call.panel, week >= 27)
```
