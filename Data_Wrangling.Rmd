...
EMS_calls.animation.data <-
  week26 %>%
    ...
    left_join(fishnet,by=c("uniqueID")) %>%
    st_sf() %>%
    mutate(Trips = case_when(Call_Count == 0 ~ "0 trips",
                             Call_Count > 0 & Call_Count <= 5 ~ "1-5 trips",
                             Call_Count > 5 & Call_Count <= 10 ~ "6-10 trips",
                             Call_Count > 10 & Call_Count <= 20 ~ "11-20 trips",
